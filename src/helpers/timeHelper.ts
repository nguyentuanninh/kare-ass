...
export const convertTimezone = (
    ...
    try {
        let targetTz = '';
        if (targetTimezone === '') {
            targetTz = currentTimezone;
        }
        let currentTimeInGivenTimezone;

        if (currentTimezone === 'UTC') {
            currentTimeInGivenTimezone = utcToZonedTime(inputTime, targetTz);
        } else {
            const currentTimezoneToUtc = zonedTimeToUtc(inputTime, currentTimezone);
            if (targetTz === 'UTC') {
                currentTimeInGivenTimezone = currentTimezoneToUtc;
            } else {
                currentTimeInGivenTimezone = utcToZonedTime(currentTimezoneToUtc, targetTz);
            }
        }
        return format(currentTimeInGivenTimezone, formatPattern, { timeZone: targetTz });
    } catch (e) {
